...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
    __cs149_vec_float x, result, clamp = _cs149_vset_float(9.999999f);
    __cs149_vec_int expVec, zero = _cs149_vset_int(0), one = _cs149_vset_int(1);
    ...
    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        ...
        _cs149_vload_int(expVec, exponents + i, maskAll);

        _cs149_vset_float(result, 1.f, maskAll);

        __cs149_mask maskExpPositive;
        _cs149_vgt_int(maskExpPositive, expVec, zero, maskAll); // expVec > 0

        // 只对expVec > 0的lane做循环
        __cs149_vec_int expLeft;
        _cs149_vmove_int(expLeft, expVec, maskExpPositive);

        __cs149_mask maskLoop = maskExpPositive;
        while (_cs149_cntbits(maskLoop) > 0) {
            _cs149_vmult_float(result, result, x, maskLoop);
            _cs149_vsub_int(expLeft, expLeft, one, maskLoop);
            _cs149_vgt_int(maskLoop, expLeft, zero, maskAll);
        }
